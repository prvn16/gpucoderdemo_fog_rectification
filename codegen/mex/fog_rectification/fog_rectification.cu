 ...
 *
 * Code generation for function 'fog_rectification'
 *
 */
  ...
  (uint8_T *restoreOut)
{
  ...
  j = (int32_T)(((((gridDim.x * gridDim.y * blockIdx.z + gridDim.x * blockIdx.y)
                  ...
                  threadIdx.z * blockDim.x * blockDim.y) + threadIdx.y *
                 blockDim.x) + threadIdx.x);
  if ((!(j >= 921600)) && ((int32_T)restoreOut[j] == 0)) {
  ...
  (real_T *expanded, real_T *diff_im)
{
  ...
  for (y_idx = srow; y_idx <= 33; y_idx += strideRow) {
    ...
    for (x_idx = scol; x_idx <= 33; x_idx += strideCol) {
      if ((baseR >= 0) && (baseR < 482) && ((baseC >= 0) && (baseC < 642))) {
  ...
  __syncthreads();
  if ((!(threadIdX >= 480)) && (!(threadIdY >= 640))) {
    cv = 0.0;
    for (n = 0; n < 3; n++) {
      for (j = 0; j < 3; j++) {
...
void fog_rectification(fog_rectificationStackData *SD, const uint8_T input
  [921600], uint8_T out[921600])
{
  int32_T idx;
  int32_T i0;
  int32_T i;
  ...
  for (i = 1; i + 3 <= 307200; i += 4) {
    if (im_gray_dirtyOnGpu) {
      cudaMemcpy((void *)&SD->f0.im_gray[0], (void *)gpu_im_gray, 307200ULL,
  ...
  if (255 - i0 < varargin_1 + 1) {
    ...
    ndbl = (int32_T)floor((254.0 - (real_T)(b_ii_size[0] + ii_size[0])) + 0.5);
    i = varargin_1 + ndbl;
    idx = (i + i0) - 254;
    absb = (int32_T)fabs(255.0 - (real_T)i0);
    u0 = varargin_1 + 1;
    if (u0 > absb) {
      absb = u0;
    }

    if (fabs((real_T)idx) < 4.4408920985006262E-16 * (real_T)absb) {
      ndbl++;
      u0 = 255 - i0;
    } else if (idx > 0) {
      u0 = varargin_1 + ndbl;
...
/* End of code generation (fog_rectification.cu) */
